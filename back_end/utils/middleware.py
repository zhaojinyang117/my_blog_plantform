    ...
    def process_request(self, request):
        ...
        if request.path.startswith("/admin/"):
            ...
            if (
                ...
                or request.path.startswith("/admin/password_reset/")
            ):
                return None

            # 对于其他admin页面，检查用户是否已认证且为管理员
            if (
                isinstance(request.user, AnonymousUser)
                or not request.user.is_authenticated
            ):
                logger.warning(
                    f"未认证用户尝试访问admin界面: {request.META.get('REMOTE_ADDR', 'unknown')} - {request.path}"
                )
                return HttpResponseForbidden("访问被拒绝：需要管理员权限")

            if not request.user.is_staff:
                logger.warning(
                    f"非管理员用户 {request.user.email} 尝试访问admin界面: {request.path}"
                )
                return HttpResponseForbidden("访问被拒绝：需要管理员权限")
...
class UserActivityMiddleware:
    ...
    def __call__(self, request):
        ...
        if (
            login_email
            and request.path == "/api/users/token/"
    ...
    def capture_login_email(self, request):
        ...
        Returns:
            str: 用户邮箱或None
        """
        if request.path == "/api/users/token/" and request.method == "POST":
            try:
                import json
    ...
    def record_user_activity(self, email, request):
        ...
        try:
            ...
            logger.info(
                f"用户 {user.email} 活动更新: IP={client_ip}, 登录次数={user.login_count}"
            )
